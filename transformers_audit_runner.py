...
from tracer.tracer import Tracer, load_config
...
jsonl_file = "audit.jsonl"
try:
    if os.path.exists(jsonl_file):
        os.remove(jsonl_file)
    with open(jsonl_file, "a", encoding="utf-8"):
        pass
except:
    print("io error")
    exit(-1)

with FileStreamWriter(jsonl_file) as stream_writer:
    with Tracer(stream_writer, config):
